...
**Server-side completeness validation.** Uploads are rejected (`400`, or `413` if oversized) unless complete: a *known* `schema_version`, non-empty reviewer name/date and dataset name, a `dataset.url` that parses as an `http(s)` URL, and a `"Yes"`/`"No"` answer for every question in that metric version. Partial templates and unknown versions can't be scored, so they're refused rather than stored half-formed. The `http(s)`-only check on `dataset.url` is also defence-in-depth: the URL is currently rendered as plain text, but pinning the scheme means a stored `javascript:`/`data:` URL can't execute if it is ever turned into a clickable link.
